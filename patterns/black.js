...
function fadeOut(leds) {
    for (var i = 0; i < leds.length; i++) {
        const bgColor = color.mix(color('red'), color('#fc5'), (Math.sin(i / 4 + new Date().getTime() / 1000) + 1) / 2 * 100);
        leds[i] = color.mix(leds[i], bgColor, 1);
    }
}
...
function traveling(leds) {
    ...
    dots.forEach((dot, index) => {
        ...
        leds[ledPos] = color.mix(dot.color, leds[ledPos], 50);
